...
import { initCheckersGame } from "./utils/init.js";
import { applyCheckersMove } from "./logic/applyMove.js";
import { triggerAIMoveIfNeeded } from "./logic/triggerAI.js";
import { CheckersService } from "./utils/services.js";
import { removePlayerFromRoom } from "./utils/removePlayers.js";

/**
 * Модульный сервер шашек
 */
export const initCheckersServer = (io: SocketServer) => {
  // ---------------------- Данные в памяти ----------------------
  ...
  /**
   * Скрытые комнаты видны только своему игроку
   */
  ...
  /**
   * Широкая рассылка комнат всем подключенным
   */
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("register_player", (player: Player) => {
      ...
      // Обновляем состояние игрока во всех комнатах если не EVE
      rooms.forEach((room) => {
    ...
    socket.on(
      ...
      (data: { gameId: string; move: Move<CheckersMove> }) => {
        const room = rooms.find((r) =>
          r.games.some((g) => g.id === data.gameId),
        );
        if (!room) return;
        const game = room.games.find((g) => g.id === data.gameId);
        if (!game) return;
