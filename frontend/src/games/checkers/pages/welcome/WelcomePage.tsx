...
export const WelcomePage = () => {
  ...
  const quickstart = () => navigate("/checkers/quickstart");
  const leaderboard = () => navigate("/checkers/leaderboard");
  const rules = () => navigate("/checkers/rules");
  const rooms = () => navigate("/checkers/rooms");

  return (
    <div className="relative h-screen w-screen overflow-hidden flex items-center justify-center px-4 pb-5">
      {/* ===== Фон ===== */}
      <img
      ...
      {/* затемнение */}
      <div className="fixed inset-0 bg-gradient-to-b from-blue-500/30 via-blue-600/30 to-indigo-800/30" />
      ...
      <div className="relative z-10 w-full max-w-[900px]">
        ...
        >
          ...
          <div className="grid grid-cols-2 grid-rows-2 gap-4 h-full">
            <MenuCard emoji="⚡" title="Быстрый старт" subtitle="Как начать играть" color="from-indigo-100 to-blue-600" onClick={quickstart} />
            <MenuCard emoji="📜" title="Правила" subtitle="Русские шашки" color="from-indigo-100 to-indigo-600" onClick={rules} />
            <MenuCard emoji="🏆" title="Рекорды" subtitle="Лучшие игроки" color="from-amber-100 to-orange-500" onClick={leaderboard} />
            <MenuCard emoji="🎮" title="Играть" subtitle="Комнаты и партии" color="from-green-300 to-green-600" onClick={rooms} />
          </div>
        </div>
      </div>
    </div>
