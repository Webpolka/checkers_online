...
export function GameHeader({
  ...
  const turnInfo = React.useMemo(() => {
    ...
    if (game.players[0].id === turnPlayer.id) {
      return { text: "БЕЛЫЕ", color: "text-black" };
    }

    if (game.players[1]?.id === turnPlayer.id) {
      return { text: "ЧЁРНЫЕ", color: "text-black" };
    }

    return { text: "ИГРОК ВЫШЕЛ", color: "text-red-600" };
  }, [currentTurnPlayerId, roomPlayers, game.players]);

  /* =====================================================
     DESKTOP (как раньше, максимум места под доску)
     ===================================================== */
  const DesktopHeader = (
